...
\begin{itemize}
    ...
    \item \texttt{RTT\_{DEV}}: Stima della variabilità di RTT determinata con:
    \[ \boxed{RTT_{DEV_{k+1}} = (1-\beta) RTT_{DEV_{k}} + \beta | RTT_{SAMPLE} - RTT_{ESTIMATED} |} \]
...
\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.325]{img/slidingWindowTCP.png}
\end{figure}
...
\begin{itemize}
    \item Slow Start
    \item AIMD (Additive Increase Multiplicative Decrease/Congestion Avoidance)
...
\paragraph{AIMD (Congestion Avoidance)} Fase in cui TCP mittente aumenta proporzionalmente la propria finestra di congestione ad ogni ACK ricevuto:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Se \texttt{cwnd > soglia} allora si incrementa \texttt{cwnd} aumenta linearmente in \textbf{Additive Increase (Congestion Avoidance)}.
    \end{itemize}
    \item \textbf{Eventi di Perdita}: Esistono varie condizioni:
    \begin{itemize}
        \item \textbf{Timeout}: Si setta \texttt{soglia = 1/2cwnd} e \texttt{cwnd = 1 MSS} in \textbf{Slow Start}.
...
\paragraph{Equità} Il TCP è equo perchè ipotizzando un bottleneck settato ad una quantità $R$ e un numero di host $k$, allora ciascuna delle connessioni TCP trasmetterà $\frac{R}{k} bit/sec$, seguendo quindi un andamento simile a quello rappresentato in figura.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.415]{img/equitaTCP.png}
\end{figure}

\newpage

\paragraph{Throughput e Connessioni TCP Parallele} Nel paragrafo precedente ipotizzavamo che ciascun applicativo aprisse esattamente una connessione TCP. Questo in un contesto reale potrebbe risultare una semplificazione, ed un applicativo potrebbe richiedere l'apertura di più connessioni TCP.
Quindi ciascuna di queste connessioni sarà alla pari delle altre, ed è a questo livello che si considera l'equità tra le connessioni stesse.

\paragraph{Attacco SYN Flood e Potenziale Mitigazione} Attacco DDoS basato sul funzionamento del handshake TCP:

\begin{itemize}
    \item L'attaccante finge di essere una moltitudine di client, i quali inviano un segmento di \texttt{SYN} al server.
    \item Il server, per handlare queste richieste, alloca un buffer per ogni \texttt{SYN} e risponde con dei \texttt{SYN-ACK}.
    \item L'attaccante non riscontra mai i \texttt{SYN-ACK} del server, lasciando le risorse del server allocate. Questo può quindi causare un esaurimento di risorse del server.
\end{itemize}

\subparagraph{Mitigazione dell'Attacco} Invece di allocare direttamente il buffer, il server genera un numero di sequenza iniziale detto \textbf{cookie}. Successivamente il server risponde con un \texttt{SYN-ACK} senza memorizzare informazioni sullo stato della connessione. Un client normale risponderà quindi con un \texttt{ACK = cookie + 1}. A questo punto il server
calcola il risultato con il valore contenuto nell'\texttt{ACK} del client, e solo se c'è corrispondenza crea la connessione ed alloca le risorse.

\subsection{Protocollo QUIC e Approfondimento HTTP/2 HTTP/3}
...
\subsubsection{HTTP/1.1 e Head of Line Blocking}

Utilizzando HTTP/1.1 in pipelining, la richiesta di grandi oggetti potrebbe ritardare 

l'ottenimento di oggetti piccoli richiesti dopo. Questo fenomeno è detto HOL Blocking (Head Of Line). Una possibile alternativa sarebbe quella di aprire \textbf{più connessioni TCP per browser}, ma questo porterebbe ad un sistema \textbf{poco equo}.
...
\begin{itemize}
    \item \textbf{Ordine di trasmissione} basata sulla \textbf{priorità} degli oggetti richiesti.
    \item \textbf{Push} di oggetti \textbf{dal server al client}, che non corrispondono a richieste esplicite dal client.
    \item \textbf{Suddivisione} dei \textbf{messaggi HTTP} in \textbf{frame HTTP}, e schedulazione dei frame per mitigare la HOL Blocking.
\end{itemize}

\newpage

\paragraph{Stream e Multiplexing} Vengono definiti dei flussi bidirezionali tra client e server 

all'interno di una singola connessione TCP. Questo meccanismo permette quindi il multiplexing di oggetti sui vari stream della connessione.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/HTTP2.png}
\end{figure}

\paragraph{Limiti dell'HTTP/2} questo approccio ottimizza l'utilizzo di risorse del protocollo TCP, ma non risolve del tutto il problema del HOL Blocking, dato che la gestione a Frame HTTP non è nota al protocollo TCP stesso. Per il TCP stanno passando solo una quantità generica di byte, senza dare a questi
ultimi un interpretazione. È infatti responsabilità degli applicativi dare una semantica alla suddivisione dei messaggi. Quindi un problema comune potrebbe essere quello dei tre timeout, che provoca uno stallo per tutti gli stream della connessione.

\subsubsection{HTTP/3 e QUIC}

Evoluzione del protocollo HTTP/2 basato sul protocollo di trasporto UDP "wrappato" dal protocollo QUIC.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/HTTP3.png}
\end{figure}

\vspace*{-5px}

\paragraph{QUIC (Quick UDP Internet Connections)} Protocollo di trasporto che sfrutta UDP ed è connection-oriented. Aggiunge al protocollo UDP elementi come:

\begin{itemize}
    \item Trasmissione affidabile, controllo di errori e congestione.
    \item Apertura delle connessioni, affidabilità, autenticazione e cifratura.
    \item Lo stato della connessione è stabilito in un solo \texttt{RTT}, a differenza di TCP che ne richiede due\footnote{Abbiamo sempre definito il costo in RTT dell'Handshake TCP come 1 \texttt{RTT}, ma in realtà vanno considerati anche il protocollo aggiuntivo per cifratura ed autenticazione TLS, che porta il costo complessivo a 2 \texttt{RTT}.}.
\end{itemize}

\newpage

\paragraph{Stream QUIC} Più stream a livello applicazione vengono multiplexati su un unica connessione QUIC. Questo garantisce trasferimento dati affidabile, controllo di flusso e controllo comune della congestione. Il concetto di stream è un astrazione al livello di trasporto, quindi ciascuno di essi è indipendente dall'altro e noto al protocollo di trasporto utilizzato.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/QUIC_streams.png}
\end{figure}

\subparagraph{Confronto Prestazioni QUIC vs TCP} In generale QUIC performa meglio di TCP, ma le performance di QUIC diminuiscono in contesti mobile e reti cellulari.

\section{Livello Rete}

L'obiettivo di questo livello di astrazione è quello di trasferire pacchetti da un host ad un altro, \textbf{offrendo servizi} al \textbf{livello di trasporto} ed \textbf{utilizzando} servizi del \textbf{livello collegamento}.
...
\subsection{Servizi e Modelli di Servizio - Forwarding/Routing}

Questo livello offre due principali servizi:

\begin{itemize}
    \item \textbf{Forwarding}: Spostamento di pacchetti dal collegamento di ingresso a quello corretto tra gli $n$ in uscita.
    \item \textbf{Routing}: Determinazione del percorso seguito dai pacchetti da sorgente a destinazione.
\end{itemize}

\paragraph{Ipotetici Servizi di Rete e Offerta Protocollo IP} I servizi di rete potrebbero offrire servizi come:

\begin{itemize}
    \item Garanzia di consegna.
    \item Consegna con garanzia temporale.
    \item Consegna in ordine.
    \item Disponibilità di banda garantita.
    \item Servizi di sicurezza.
\end{itemize}

Il protocollo IP è di tipo \textbf{best-effort} da questo punto di vista, quindi \textbf{non garantisce alcuna di queste proprietà}\footnote{Esistono degli standard basati su IP, come \texttt{INTSERV} e \texttt{DIFFSERV} che garantiscono o possono garantire proprietà come tempo, banda, ordine e perdite. }.

\newpage

\subsection{Protocollo IP}

Protocollo di tipo connection-less, non esiste alcun circuito fisico o virtuale tra host sorgente e destinazione. In origine il livello di rete si riferiva esclusivamente a questo protocollo, attualmente
invece attualmente si compone di più protocolli.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.6]{img/livelloReteRappresentazione.png}
\end{figure}

Quindi come definito prima non si ha nessun tipo di garanzia sulla qualità del servizio.

\subsubsection{Funzioni IP}

Come già definito prima, il protocollo IP fornisce due principali tipi di funzioni:

\begin{itemize}
    \item \textbf{Forwarding (Inoltro)}: Trasferimento del pacchetto sull'appropriato collegamento di uscita.
    \begin{figure}[htbp]
        \center
        \includegraphics[scale=0.33]{img/forwardingIP.png}
    \end{figure}
    Questa tabella d'inoltro va però prima popolata, questa si determina infatti tramite la funzione di instradamento.
    \item \textbf{Instradamento}: Processo \textbf{decisionale} di scelta del percorso verso una destinazione, tramite \textbf{algoritmi di routing}.
    \begin{figure}[htbp]
        \center
        \includegraphics[scale=0.33]{img/instradamentoIP.png}
    \end{figure}
\end{itemize}

\newpage

\subsubsection{Meccanismi IP}

Il protocollo IP definisce due meccanismi:

\begin{itemize}
    \item \textbf{Indirizzamento}: Strumento per identificare gli host.
    \item \textbf{Modello Datagram}: Consegna dei dati in segmenti basata su specifiche fasi:
    \begin{itemize}
        \item Frammentazione e riunificazione dei pacchetti.
        \item Controllo degli errori.
        \item Verifica TTL\footnote{Con TTL si intende Time To Live, ossia quanti salti il pacchetto può fare al massimo.}.
    \end{itemize}
    \item \textbf{Multiplexing/Demultiplexing}: Anche al livello di rete si effettua multiplexing, infatti grazie ad un campo presente nel datagramma IP si riesce a capire a quale protocollo del livello superiore debba essere inviata l'informazione decapsulata.
\end{itemize}

\subsubsection{Formato Datagramma IP}
...
\hline
\multicolumn{1}{|c|}{\textbf{4 bits}} &
\multicolumn{1}{c|}{\textbf{4 bits}} &
\multicolumn{1}{c|}{\textbf{8 bits}} &
\multicolumn{1}{c|}{\textbf{16 bits}} \\
\hline
Version & Header Length & Type of Service & Total Length \\
\hline
\multicolumn{3}{|c|}{Identification (16 bits)} & \\
\cline{1-3}
Flags (3 bits) & \multicolumn{2}{c|}{Fragment Offset (13 bits)} & \\
\cline{1-3}
\multicolumn{1}{|c|}{Time To Live (8 bits)} &
\multicolumn{1}{c|}{Protocol (8 bits)} &
\multicolumn{2}{c|}{Header Checksum (16 bits)} \\
\hline
\multicolumn{4}{|c|}{Source IP Address (32 bits)} \\
\hline
\multicolumn{4}{|c|}{Destination IP Address (32 bits)} \\
\hline
\multicolumn{4}{|c|}{Options (variable, if any)} \\
\hline
\multicolumn{4}{|c|}{Data (variable length, typically TCP/UDP segment)} \\
\hline
\end{tabular}
\caption{Formato del Datagramma IP (IPv4 Header)}
\end{table}

\begin{itemize}
    \item \textbf{Versione}: Indica la versione utilizzata del protocollo IP.
    \item \textbf{Header Length}: Lunghezza dell'intestazione espressa in parole da \texttt{32 bit}.
    \item \textbf{Service Type}: Permettono la caratterizzazione di Datagrammi IP:
    \begin{itemize}
        \item \textbf{Differentiated Services}: I pacchetti vengono caratterizzati per la loro classe di servizio o tipi di accodamento ai router (\texttt{6 bit}).
        \item \textbf{Explicit Congestion Notification (ECN)}: Supporto a livello rete della gestione della congestione a livello di trasporto (\texttt{2 bit}).
    \end{itemize}
    \item \textbf{Lunghezza del datagramma (16 bit)}: è la lunghezza di tutto il datagramma in byte, header incluso.
    \begin{itemize}
        \item Dim. Max 65535 byte (nella pratica 1500 byte)
    \end{itemize}
    \item \textbf{Identificatore, flag, offset}: sono campi per la frammentazione (vedi più avanti).
    \item \textbf{Tempo di vita (8 bit)}: ad ogni passaggio da un router viene decrementato, quando raggiunge lo zero viene scartato. Assicura che eventuali percorsi ad anello non 
    
    provochino traffico perpetuo nella rete.
        \begin{itemize}
            \item Valore iniziale dipende dal S.O.: esempi 30, 64, 128, 255
        \end{itemize}
    \item \textbf{Protocollo (8 bit)}: in ricezione all'host destinatario indica quale protocollo dello strato superiore deve ricevere i dati. Inizialmente la tabella era nella RFC1700 (STD2), attualmente è un database online che si trova su iana.org
        \begin{itemize}
            \item Demux, es. TCP 6, UDP 17
        \end{itemize}
    \item \textbf{Checksum dell'intestazione (16 bit)}: viene calcolato il checksum della sola intestazione (ponendo questo campo pari a zero) ad ogni router (il TTL cambia ad ogni hop!). Se si ottiene un errore si scarta il datagramma.
    \item \textbf{Indirizzi sorgente e destinazione (32 bit)}: indirizzi IP del mittente e del destinatario.
    \item \textbf{Opzioni (lunghezza variabile, multiplo di 32 bit)}: uso sporadico, da 0 a 40 byte
    \item \textbf{Dati}: dati trasportati dal datagramma IP.
\end{itemize}

\subsubsection{Frammentazione di Datagramma IP}

Dato che il livello rete dipenderà dal livello collegamento dovrà scendere a compromessi con la dimensione Maximum Transfer Unit (MTU), ossia la dimensione massima di invio del collegamento. Quindi l'idea è quella di suddividere un \textbf{Datagramma IP} in tanti \textbf{segmenti diversi}, che dovranno poi essere riassemblati
nel sistema di destinazione.

\vspace*{10px}

Ovviamente questa gestione si porta dietro vari \textbf{punti critici}:

\begin{itemize}
    \item Se uno solo dei frammenti si perde, il sistema terminale è costretto a scartare tutto il resto del datagramma ricevuto.
    \item I frammenti possono arrivare fuori ordine e di conseguenza vanno riordinati tramite \textbf{identificazione}, \textbf{offset} e\textbf{flag}:
    \begin{itemize}
        \item \textbf{Identificazione}: Mantiene un riferimento al \textbf{datagramma IP originale}, conservando anche IP sorgente e IP destinazione.
        \item \textbf{Offset}: Indica la posizione relativa al frammento nel datagramma in multipli di \texttt{8 byte}.
        \begin{figure}[htbp]
            \center
            \includegraphics[scale=0.35]{img/offsetFrammentoDiDatagrammaIP.png}
        \end{figure}
        \item \textbf{Flag}: Tre bit dedicati a dettagli specifici:
        \begin{itemize}
            \item \textbf{Primo Bit}: Bit riservato.
            \item \textbf{Secondo Bit}: Not fragment, se settato ad 1 non si frammenta il datagramma.
            \item \textbf{Terzo Bit}: Se settato ad 1 indica che quello è l'ultimo frammento del datagramma.
        \end{itemize}
    \end{itemize}
\end{itemize}

Dati tutti questi elementi di complessità, il \textbf{processo di frammentazione è critico}, quindi in tante occasioni si preferisce non frammentare.

\subsubsection{Indirizzi IP}

Ogni host è connesso ad Internet tramite un \textbf{interfaccia di rete}, ossia il \textbf{confine tra host e collegamento} su cui vengono trasmessi i datagrammi.
Ogni indirizzo IP, secondo il protocollo IPv4, è definito da \texttt{4 byte}, quindi 32 bit che solitamente vengono riportati nella seguente notazione decimale puntata:

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.55]{img/IPv4.png}
\end{figure}

Qgnuno di questi indirizzi si compone di due parti, ossia un \textbf{Network ID} ed un \textbf{Host ID}, che identificano rispettivamente una rete su Internet e l'host su quella rete IP.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.55]{img/IPv4_2.png}
\end{figure}

L'assegnazione di specifici bit alla rete ed i restanti agli host determina un tipo di indirizzamento detto \textbf{Classful Addressing}.

\newpage

\subsubsection{Classful/Classless Addressing e Subnet Mask}

Definiamo delle \textbf{classi di indirizzi IPv4} in base ai byte dedicati all'identificazione di reti:

\begin{itemize}
    \item \textbf{Indirizzamento Classful}:
    \begin{itemize}
        \item \textbf{Classe A}: Si dedica il primo byte all'identificazione di una rete e i restanti tre per gli host.
        \item \textbf{Classe B}: Si dedicano due byte alla rete e gli altri due per gli host.
        \item \textbf{Classe C}: Si dedicano tre byte alla rete ed il restante per gli host.
        \item \textbf{Classe D}: Riservata al multicasting.
        \item \textbf{Classe E}: Riservata ad usi futuri.
    \end{itemize}
    \item \textbf{Indirizzamento Classless}: Definiamo un modo custom per indicare il numero di bit dedicati alla rete, ossia \texttt{x.x.x.x/num\_bit\_rete}.
\end{itemize}

\paragraph{Subnet Mask} Basandoci sulla notazione classless, possiamo definire una maschera di sottorete, che se messa in \texttt{AND} con un indirizzo IPv4 permette di ricavare l'indirizzo della rete:

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/subnetMask.png}
\end{figure}

\subsubsection{Assegnazione a Blocchi e Indirizzi Speciali}

\paragraph{Assegnazione Blocchi di Indirizzi} Se un ISP deve suddividere un blocco in $8$ blocchi di indirizzi.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/assegnazione_a_blocchi.png}
\end{figure}

\begin{itemize}
    \item Il numero di indirizzi N in ogni sottoreti deve essere una potenza di 2.
    \item La lunghezza del prefisso di ogni sottorete va calcolata con la seguente formula $n = 32 - log_2 N$ dove $N$ è il numero di indirizzi della sottorete.
    \item Si assegnano blocchi di indirizzi contigui.
    \item Se i blocchi sono di dimensioni diverse allora si parte dai blocchi più grandi.
\end{itemize}

\newpage

\paragraph{Indirizzi Speciali} Esiste una serie di indirizzi riservati:

\begin{itemize}
    \item \texttt{This-Host 0.0.0.0}: Usato come placeholder quando un host ancora non conosce il proprio indirizzo IP.
    \item \texttt{Limited Broadcast 255.255.255.255}: Usato quando un router o un host deve inviare un pacchetto a tutti gli host in una rete.
    \begin{itemize}
        \item \texttt{Directed Broadcast}: Serve per indirizzare tutti i nodi di una specifica sottorete esterna.
    \end{itemize}
    \item \texttt{Loopback 127.0.0.1} Il datagramma con questo indirizzo di destinazione non lascia l'host locale.
\end{itemize}

\subsubsection{IP Forwarding - Diretto vs Indiretto}

Ogni diagramma IP è soggetto a forwarding, ossia inoltro verso l'uscita. Esistono due tipi diversi di inoltro:

\begin{itemize}
    \item \textbf{Inoltro Diretto}: Il pacchetto IP ha come destinazione un host nella propria rete, quindi \textbf{non viene interpellato alcun} router di quella rete.
    \begin{itemize}
        \item L'Host A deve inviare un pacchetto ad un Host B, conoscendone l'indirizzo IP.
        \item L'Host A confronta il proprio indirizzo con quello del destinatario, capisce che si tratta di un host della stessa sottorete e quindi sceglie un inoltro diretto.
        \item L'Host A consulta quindi una tabella di corrispondenza \texttt{IP\_ADD -> MAC\_ADD}.
        \item L'Host A passa il pacchetto al livello di trasporto, che gestirà il frame con header contenente gli indirizzi MAC sorgente e destinatario.
    \end{itemize}
    \item \textbf{Inoltro Indiretto}: Il pacchetto IP ha come destinazione un host di un'altra rete IP. Quindi il pacchetto viene inoltrato \textbf{almeno ad un} router.
    \begin{itemize}
        \item L'Host A deve inviare un pacchetto ad un Host B, conoscendone l'indirizzo IP.
        \item L'Host A confronta il proprio indirizzo con quello del destinatario, capisce che si tratta di un host di un altra sottorete e quindi sceglie un inoltro indiretto.
        \item L'Host A quindi recupera l'indirizzo MAC del router dalla tabella di corrispondenza e passa il pacchetto a livello inferiore in sua direzione.
        \item Il pacchetto viene costruito e spedito dall'interfaccia del router.
    \end{itemize}
\end{itemize}

\vspace*{5px}

\paragraph{Longest Prefix Matching} Quando si cerca una voce della tabella di inostro per un dato indirizzo di destinazione, in caso di più match nella tabella, si seleziona la entry con il prefisso di indirizzo più lungo tra tutte quelle che matchano. Questo perchè
un prefisso più lungo per la rete garantisce che sia più specifico rispetto agli altri match.

\newpage

\paragraph{Aggregazione di Indirizzi e Routing Gerarchico} Assumiamo che un ISP abbia assegnato $4$ piccoli blocchi di indirizzi ad organizzazioni. L'ISP può aggregare questi blocchi in uno singolo e annuncia alla rete il singolo blocco per questioni di efficienza.

\subsection{Protocollo DHCP - Dynamic Host Configuration Protocol}

Un indirizzo IP può essere \textbf{configurato manualmente} tramite assegnazione di indirizzo IP, indirizzo gateway/router, netmask e indirizzo IP di un server DNS.
...
\paragraph{Protocollo DHCP secondo Paradigma Client/Server} L'assegnazione dinamica si basa su queste fasi:

\begin{itemize}
    \item L'host invia in broadcast un messaggio \texttt{DHCP discover}.
    \item Il server DHCP risponde con un messaggio \texttt{DHCP offer}.
    \item L'host richiede un indirizzo IP tramite un messaggio \texttt{DHCP request}.
    \item Il server DHCP invia un messaggio \texttt{DHCP ack} se la richiesta va a buon fine.
\end{itemize}

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.5]{img/DHCP1.png}
\end{figure}

Il DHCP il basa sul protocollo UDP e non si occupa solo di assegnazioni di IP ma anche di informazioni riguardanti indirizzo gateway/router, netmask ed indirizzo IP di almeno un server DNS.

\newpage

\subsection{NAT - Network Address Translation}

Questo meccanismo permette di trasmettere su Internet il traffico proveniente da sistemi di sottoreti private, dove l'accesso ad Internet è \textbf{abilitato} da un \textbf{router} che ha abbia \textbf{almeno} un \textbf{IP pubblico}.
...
\paragraph{Tabella di Traduzione NAT} Il router di accesso alla sottorete ha quindi in memoria una tabella di associazioni per riferire ad un host specifico della sua sottorete.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.55]{img/tabellaNAT.png}
\end{figure}

\paragraph{Critiche al NAT} Il NAT è stato visto in modo controverso da chi definiva formalmente le specifiche di Internet per questioni quali ad esempio l'utilizzo di porte per riferire host e non processi, violazione dell'argomento end-to-end, carenza di indirizzi già risolta da IPv6 e attraversamento del NAT per connettersi ad un server nattato.

\subsection{ICMP - Internet Control Message Protocol}

Il \textbf{meccanismo IP} di base \textbf{non ha} alcun meccanismo integrato per \textbf{notifica di errori}. Oltre a questo è anche sprovvisto di meccanismi per effettuare richieste sullo stato di un sistema remoto.

\paragraph{Alcune Caratteristiche Messaggi ICMP} I pacchetti ICMP sono incapsulati all'interno di datagrammi IP. Per pacchetti IP frammentati, i messaggi ICMP sono relativi solo al frammento con offset $0$. I messaggi ICMP non sono mai inviati in risposta a pacchetti IP con destinazione IP broadcast o multicast, e allo stesso tempo non sono mai inviati in risposta ad altri messaggi di errore ICMP ma possono essere utilizzati in risposta a messaggi ICMP di interrogazione.

\newpage

\paragraph{Tipi di Messaggio ICMP} I messaggi si distinguono in:

\begin{itemize}
    \item \textbf{Messaggi di Segnalazione Errore}
    \item \textbf{Messaggi di Richiesta/Risposta}
\end{itemize}

Oltre alla tipologia, ogni messaggio ICMP è caratterizzato anche da due attributi 

\texttt{tipo 8bit} e \texttt{codice 8bit}, che indicano il significato dei messaggi.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.6]{img/ICMP_messages.png}
\end{figure}

\paragraph{Esempi di Utilizzo ICMP - Ping e Traceroute} Descriviamo l'utilizzo di questi due meccanismi che si basano su messaggi ICMP:

\begin{itemize}
    \item \textbf{Ping}: Si basa su uno scambio di messaggi ICMP richiesta/risposta di tipo \texttt{ECHO}, rispettivamente \texttt{tipo 8, codice 0} dal mittente e \texttt{tipo 0, codice 0} dal destinatario se raggiungibile.
    \begin{enumerate}
        \item \textbf{Versione UNIX}: Traceroute si basa sulla configurazione che permette di 
        
        scatenare l'invio di messaggi di errore correlati a tempo scaduto o porte di destinazione non raggiungibili.
        \item \textbf{Versione Windows}: Tracert utilizza invece messaggi echo ICMP di tipo request/reply.
        \begin{figure}[htbp]
            \center
            \includegraphics[scale=0.4]{img/tracert.png}
        \end{figure}
    \end{enumerate}
\end{itemize}
...
\subsection{Architettura di Router}

Come già accennato, un router deve occuparsi di due tipologie di operazione:

\begin{itemize}
    \item \textbf{Forwarding}: Trasferire i pacchetti sull'appropriato collegamento in uscita, \textbf{utilizzando} una \textbf{tabella di inoltro}, l'operazione è detta anche di tipo \textbf{data plane}.
    \item \textbf{Routing}: Processo \textbf{decisionale} di scelta del percorso verso una destinazione, \textbf{popola} quindi la \textbf{tabella di inoltro}, azione definita da algoritmi di routing, definita anche come di tipo \textbf{control plane}.
\end{itemize}

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/dataPlane_controlPlane.png}
\end{figure}

Possiamo quindi definire due tipi di routing:

\begin{itemize}
    \item \textbf{Routing Decentralizzato}: L'algoritmo di routing è eseguito su ciascuno dei router, e questi ultimi si scambiano messaggi.
    \item \textbf{Routing Logicamente Centralizzato}: Un controller remoto con Control Agents \texttt{CA} locali si occupa delle informazioni sui collegamenti e sul traffico, ed invia ai singoli \texttt{CA} i valori da inserire nella tabella di inoltro. Questo tipo di gestione è definita
    anche come \textbf{Software-defined Networking SDN}.
\end{itemize}

\subsubsection{Descrizione Fisica dell'Architettura di un Router}
...
\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/archRouter1.png}
\end{figure}

\newpage

\begin{itemize}
    \item \textbf{Porte di Input}: Si compone di tre parti, una ricezione dei bit, un livello di collegamento ed un modulo per la gestione di lookup, forwarding ed accodamento.
    \begin{figure}[htbp]
        \center
        \includegraphics[scale=0.4]{img/archRouter2.png}
    \end{figure}
    \item \textbf{Porte di Output}: Si occupa del buffering dei datagrammi quando arrivano dalla struttura di commutazione ad una velocità maggiore rispetto a quella di trasmissione del collegamento in uscita. Oltre a questo devono anche definire delle drop policy per scegliere quali datagrammi scartare in caso di mancanza di spazio e politiche di scheduling per definire l'ordine di trasmissione dei datagrammi in coda.
    \begin{figure}[htbp]
        \center
        \includegraphics[scale=0.4]{img/archRouter3.png}
    \end{figure}
    \item \textbf{Struttura di Commutazione}: Si occupa del trasferimento del pacchetto dal buffer di input al buffer di output appropriato, caratterizzata da una velocità con cui i pacchetti possono essere trasferiti da un buffer ad un altro.
    \begin{figure}[htbp]
        \center
        \includegraphics[scale=0.4]{img/archRouter4.png}
    \end{figure}
\end{itemize}

\paragraph{Ritardi e Perdite} Si possono formare delle code di pacchetti sia presso porte di ingresso che di uscita, la lunghezza delle code dipende da vari fattori come traffico di rete, velocità della struttura di comunicazione e velocità della linea.
...
\paragraph{Astrazione del Grafo} Immaginiamo che la rete sia un grafo, ciascun arco ha un peso, quindi ci chiediamo quale sia il \textbf{cammino di costo minimo} tra sorgente e destinazione. Un \textbf{algoritmo di routing calcola} quindi il \textbf{cammino} a \textbf{costo minimo} tra i due nodi.

\paragraph{Routing Statico/Dinamico} Esistono due tipi di routing:

\begin{itemize}
    \item \textbf{Statico}: Un admin definisce il path ottimale, popolando manualmente le entry delle tabelle di routing. Questo approccio viene utilizzato per reti di piccole dimensioni.
    \item \textbf{Dinamico}: Definito da protocolli specifici che popolano automaticamente le tabelle di routing con i possibili percorsi. Utilizzato in reti di media/grande dimensione.
\end{itemize}

\paragraph{Algoritmi di Routing Globali/Decentralizzati} Classifichiamo gli algoritmi di instradamento in:

\begin{itemize}
    \item \textbf{Globali}: Basati sulla conoscenza dell'intera topologia di rete, il calcolo può essere fatto in un unico sito, utilizzando informazioni su tutti i nodi e gli archi. Ne è un esempio l'\textbf{algoritmo Link State}.
    \item \textbf{Decentralizzati}: Nessun nodo conosce l'intera topologia di rete, di conseguenza si distribuisce il calcolo e si reitera. Ne è un esempio l'\textbf{algoritmo Distance Vector}.
\end{itemize}
...
\paragraph{Equazione di Bellman-Ford} Formula per trovare il percorso a costo minimo tra un nodo sorgente $x$ ed una destinazione $y$ attraversando nodi intermedi, detti vicini $v$.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.4]{img/equazioneBellmanFord.png}
\end{figure}

\begin{itemize}
    \item \textbf{Costo}: Definito un costo $c(x,v)$ tra nodo sorgente $x$ e vicino $v$.
    \item \textbf{Distanza Minima $d_{vy}$}: Sia nota la distanza minima tra vicino $v$ e destinazione $y$.
\end{itemize}

Da questo definiamo quindi:

\[ \boxed{d_{xy} = min_v\{ c(x,v) + d_{vy} \}} \]

dove $c(x,v)$ è il costo da $x$ al vicino $v$ e $d_{vy}$ è la distanza dal vicino $v$ alla destinazione $y$.

\newpage

\paragraph{Funzionamento Algoritmo Distance Vector} L'algoritmo del Distance Vector si basa su queste fasi:

\begin{itemize}
    \item Ogni nodo calcola il proprio vettore distanza e ne invia una copia a ciascuno dei suoi vicini.
    \item Quando un nodo riceve un vettore distanza, lo salva ed applica l'equazione di Bellman-Ford per aggiornare il proprio vettore distanza, quindi:
    \[ D_{xy} = min_v \{ c(x,v) + D_{vy} \} \: \text{per ogni} \: y \in N \]
    \item Finchè tutti i nodi continuano a cambiare i propri Distance Vector in maniera asincrona, ciascuna stima dei costi $D_{xy}$ converge all'effettivo costo del percorso a costo minimo $d_{xy}$.
\end{itemize}

Per questo ad inizio sottocapitolo si definiva questo algoritmo distribuito, ciascun nodo infatti:

\begin{enumerate}
    \item Attende un \textbf{cambiamento di costo} di un \textbf{arco locale} oppure un \textbf{messaggio da un vicino}.
    \item \textbf{Ricalcola} la \textbf{stima} dei \textbf{propri Distance Vector} usando il Distance Vector ottenuto dal vicino.
    \item Se il \textbf{proprio Distance Vector varia} allora lo \textbf{notifica} ai \textbf{vicini}.
\end{enumerate}

\paragraph{Notifica Cambiamento Costo} Questo algoritmo ha due diversi comportamenti in caso di variazione di costo degli archi:

\begin{itemize}
    \item \textbf{Decremento - "Good News Travels Fast"}: Se il cambiamento del costo di un arco è un decremento, allora questa informazione è facilmente propagata ai nodi vicini.
    \item \textbf{Incremento - "Bad News Travels Slow"}: Se il cambiamento del costo è un incremento si ricade nella problematica del \textbf{count to infinity}, sono necessarie molte iterazioni prima che l'algoritmo si stabilizzi. Questa condizione si risolve con due approcci, che possono anche essere combinati:
    \begin{itemize}
        \item \textbf{Split Horizon}: Tutte le rotte ricevute tramite un interfaccia \textbf{non devono} essere ritrasmesse a quell'interfaccia.
        \item \textbf{Poisoned Reverse}: Se un nodo $x$ trasmette pacchetti a $v$ che sono diretti ad $y$ allora li etichetta come irragiungibili da $x$, passando ad esempio un etichetta $\infty$.
    \end{itemize}
\end{itemize}

\subsubsection{Algoritmo Link-State}

Tipo di \textbf{algoritmo globale}, la \textbf{topologia di rete} ed i suoi collegamenti sono \textbf{noti a tutti} i nodi. Si calcola quindi il cammino a costo minimo da un nodo origine a tutti gli altri nodi tramite l'utilizzo dell'\textbf{Algoritmo di Dijkstra}.

\newpage

\paragraph{Notazione Algoritmo di Dijkstra} Utilizzeremo questa notazione:

\begin{itemize}
    \item $c(x,y)$: Costo dei collegamenti dal nodo $x$ al nodo $y$, settato ad $\infty$ se non sono collegati.
    \item $D(v)$: Costo del cammino dal nodo origine alla destinazione $v$ per l'iterazione corrente.
    \item $p(v)$: immediato predecessore di $v$ lungo il cammino.
    \item $N'$: Sottoinsieme di nodi per cui il cammino a costo minimo dall'origine è definitivamente noto.
\end{itemize}

\paragraph{Complessità Algoritmo di Dijkstra} Dati $n$ nodi, per ogni iterazione è necessario controllare $w$ non presenti in $N$. La complessità è quindi $O(n^2)$ ma esistono versione che scendono ad $O(nlogn)$. Da un punto di vista dei messaggi, ogni router deve trasmettere le informazioni sullo stato del proprio collegamento ad altri $n$ router.

\subsubsection{Confronto Distance Vector e Link State}

Analizziamo alcune caratteristiche mettendo a confronto i due algoritmi:

\begin{itemize}
    \item \textbf{Complessità Messaggi}:
    \begin{itemize}
        \item \textbf{Distance Vector}: Richiede scambi tra nodi adiacenti.
        \item \textbf{Link State}: Con $n$ nodi richiede l'invio di $O(n^2)$ messaggi.
    \end{itemize}
    \item \textbf{Velocità di Convergenza}:
    \begin{itemize}
        \item \textbf{Distance Vector}: Può convergere lentamente, può presentare cicli 
        
        nell'instradamento e problemi di conteggio all'infinito.
        \item \textbf{Link State}: L'algoritmo $O(n^2)$ richiede $O(n^2)$ messaggi.
    \end{itemize}
    \item \textbf{Robustezza}: Immaginiamo una condizione in cui un router funzioni male:
    \begin{itemize}
        \item \textbf{Distance Vector}:
        \begin{enumerate}
            \item Un nodo può comunicare cammini a costo minimo errati a tutte le destinazioni.
            \item La tabella di ciascun nodo può essere usata dagli altri.
            \item Un calcolo errato si può diffondere per l'intera rete.
        \end{enumerate}
        \item \textbf{Link State}:
        \begin{itemize}
            \item Un router potrebbe inviare in broadcast un costo sbagliato per uno dei suoi collegamenti.
            \item In nodi si occupano di calcolare soltanto le proprie tabelle.
        \end{itemize}
    \end{itemize}
\end{itemize}
...
\subsection{Sistemi Autonomi AS ed Instradamento Gerarchico}

Sarebbe irrealistico immaginare un routing basato sulla visualizzazione di tutti i miliardi di hosts che compongono Internet. Tutta la rete è composta da sistemi autonomi \textbf{AS}, ossia un \textbf{gruppo
di router} sotto lo \textbf{stesso controllo amministrativo}.

\paragraph{Tipi di AS} Elenchiamo i vari tipi di sistemi autonomi:

\begin{itemize}
    \item \textbf{AS Stub}: Collegato solo ad un altro stub, trasporta solo traffico di cui è origine o destinazione.
    \item \textbf{AS Multihomed}: Collegato a più di un altro AS.
    \item \textbf{AS di Transito}.
\end{itemize}

\subsubsection{Intra/Inter-AS}

Si differenzia il tipo di routing in base a se sorgente e destinazione facciano o meno parte dello stesso AS:

\begin{itemize}
    \item \textbf{Intra Domain Routing}: Routing all'interno dello stesso AS, tutti i router al suo interno utilizzano lo stesso protocollo intra-domain. Un \textbf{gateway router} si trova al confine del suo AS ed ha i link con i router di altri AS.
    \item \textbf{Inter Domain Routing}: I gateway router si occupano dell'instradamento inter-domain, quindi tra AS diversi.
\end{itemize}

\vspace*{10px}

\subsection{Protocolli Reali per Intra-AS}
...
\begin{itemize}
    \item Basato su Distance Vector con Poisoned Reverse.
    \item Gestione del \textbf{costo} basata sul \textbf{numero di hop} effettuati tra sottoreti per raggiungere la destinzazione.
    \item Basato su richieste/risposte.
    \item Utilizza un processo demone che comunica tramite protocollo UDP con porta standard $520$.
    \item Non viene inviata solo la distance vector ma tutta la tabella di routing.
\end{itemize}

\newpage

\paragraph{Algoritmo del RIP} Un nodo invia la tabella di inoltro ai propri vicini in maniera periodica, solitamente 30 secondi. Si definisce quindi il costo di hop tra $R$ ed $Y$ con:
...
\begin{itemize}
    \item $D_{R}[Y]$: Corrisponde alla nuova distanza tra $R$ sorgente ed $Y$ destinazione.
    \item $D_{V}[Y]$: Corrisponde alla distanza che il nodo $V$ afferma di avere per raggiungere la destinazione $Y$.
    \item $+1$: Costo causato dal collegamento tra i due nodi vicini, ossia $R$ e $V$.
\end{itemize}
...
\paragraph{Prestazioni RIP} Così come per l'algoritmo DV abbiamo una potenziale convergenza lenta delle tabelle di inoltro, specialmente in domini ampi. Allo stesso tempo però si garantisce robustezza in caso di guasto di un router a causa della sua logica distribuita.

\paragraph{Protocollo RIP e Utilizzo UDP} Nella precedente pagina abbiamo definito come questo protocollo a livello di rete si basi sull'utilizzo dell'UDP, che è un protocollo di trasporto. Non stiamo però nell'ambito del data plane ma del control plane, quindi non siamo limitati solo all'utilizzo del livello di rete, ma possiamo anche arrivare fino al livello applicazione. Quindi non si è limitati solo al livello di rete nei router per il Control Plane, proprio perchè il compito è la comunicazione per la creazione delle regole, e non per l'inoltro effettivo.

\subsubsection{OSPF - Open Shortest Path First}

Basato sul Link State, in maniera periodica viene effettuata una operazione di \textbf{flooding} che permette la propagazione dello stato globale della rete.

\paragraph{Caratteristiche OSPF} Elenchiamo alcune caratteristiche di questo protocollo:

\begin{itemize}
    \item Il payload OSPF viene incapsulato in datagrammi IP.
    \item Questo protocollo è utilizzato soprattutto in reti grandi, per evitare quindi di inondare tutta la rete di messaggi si effettua l'invio in maniera globale ma relativamente ad un area/dorsale della rete totale.
\end{itemize}

\paragraph{Prestazioni OSPF} Definiamo le prestazioni di questo protocollo:

\begin{itemize}
    \item Il formato dei payload e il metodo di trasmissione a flooding potrebbero generare molto traffico e sprecare banda.
    \item La convergenza delle tabelle di inoltro è molto più rapida del RIP.
    \item La robustezza è garantita dall'indipendenza ci ciascun router.
\end{itemize}

\newpage

\subsection{Protocolli Reali per Inter-AS}
...
Oltre a questo gestisce in maniera aggregata gli indirizzi e distribuisce \textbf{informazioni di raggiungibilità} in due modi:

\begin{itemize}
    \item \textbf{eBGP - external BGP}: Gestione dello scambio di informazioni di routing tra AS diversi, tramite comunicazione dei router Gateway con altri router Gateway di AS esterni.
    \item \textbf{iBGP - internal BGP}: Gestione della distribuzione interna delle informazioni sulle rotte apprese dall'esterno dal Gateway del AS locale.
\end{itemize}

\paragraph{Messaggi BGP}

Due router BGP, detti peer, si scambiano messaggi BGP su una connessione TCP semi-permanente, ciascun nodo mantiene quindi informazioni sul percorso completo verso una destinazione, non solo sulla distanza o sulla prossima tappa. Esistono vari tipi di messaggi BGP, ad esempio \texttt{OPEN}, \texttt{UPDATE}, \texttt{KEEP ALIVE}, \texttt{NOTIFICATION}.

\paragraph{Annunci ed Attributi BGP} Definiamo elementi principali di questo meccanismo:

\begin{itemize}
    \item \textbf{Annuncio BGP}: Messaggio centrale che un router BGP invia ai vicini per comunicare una rotta verso una o più destinazioni.
    \item \textbf{Prefisso CIDR}\footnote{Classless Inter Domain Routing}: Identifica la destinazione raggiungibile come blocco di indirizzi IP rappresentato in notazione CIDR.
    \item \textbf{Attributo AS-PATH}: Lista logica ordinata degli AS attraversati per raggiungere il corrente router.
    \item \textbf{Attributo NEXT-HOP}: Indirizzo IP del prossimo router fisico che il pacchetto deve raggiungere.
\end{itemize}

\paragraph{Selezione Rotta con BGP} Un router potrebbe \textbf{conoscere più di un percorso} verso l'\textbf{AS destinazione}, quindi questo viene selezionato in base a vari elementi come una \textbf{preferenza locale} assegnata alle rotte, l'\textbf{AS-PATH più breve}, il \textbf{NEXT-HOP più vicino} oppure \textbf{altri criteri aggiuntivi}.

\newpage

\paragraph{Policy Based Routing} Il gateway che riceve l'annuncio di una rotta usa una import policy per accettare/rifiutare un percorso. Le policy vengono utilizzate anche per pubblicizzare o meno un percorso ad altri AS vicini.

\paragraph{Riassunto Caratteristiche Protocolli Inter/Intra AS} Elenchiamo le caratteristiche mostrate negli ultimi due capitoli:

\begin{itemize}
    \item \textbf{Politiche}:
    \begin{itemize}
        \item \textbf{Inter-AS}: Si vuole che l'amministratore abbia controllo su come viene instradato il suo traffico.
        \item \textbf{Intra-AS}: Singolo amministratore, la gestione della politica è meno problematica.
    \end{itemize}
    \item \textbf{Scalabilità}: L'instradamento gerarchico consente di ridurre le dimensioni delle tabelle e del traffico di aggiornamento.
    \item \textbf{Performance}:
    \begin{itemize}
        \item \textbf{Inter-AS}: Si tiene conto delle performance.
        \item \textbf{Intra-AS}: Le politiche hanno maggiore importanza delle performance.
    \end{itemize}
\end{itemize}

\subsection{Protocollo IPv6}
...
\paragraph{Caratteristiche} Questo protocollo si basa su queste caratteristiche:

\begin{itemize}
    \item Indirizzi a 128 bit.
    \item Lunghezza dell'header fissa a 40 byte.
    \item Eliminazione della checksum.
    \item Nessuna frammentazione.
\end{itemize}

\paragraph{Tunneling} Dato che non è possibile switchare da IPv4 ad IPv6 si definisce un meccanismo di "convivenza" tra i due protocolli, grazie al quale si wrappa tutto il datagramma IPv6 nel payload del datagramma IPv4 in modo tale che questo possa essere trasmesso in una regione IPv4.

\begin{figure}[htbp]
    \center
    \includegraphics[scale=0.575]{img/tunnelingIPv6.png}
\end{figure}
